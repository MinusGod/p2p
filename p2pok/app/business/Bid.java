...
public class Bid implements Serializable{
	...
	public double releaseLevel = 0d; // 放款百分比   add by xd.wang
	...
	public String getStrStatus() {
		if(null == this.strStatus){
			switch (this.status) {
				...
				case Constants.BID_RELEASEING: this.strStatus = "还款中"; break;
				//已还款--->已完成 modify by xd.wang
				case Constants.BID_REPAYMENTS: this.strStatus = "已完成"; break;
	...
	public void setId(long id) {
		...
		//管理费 add by xd.wang
		this.managerFee = tbid.manager_fee;
	...
	public void createBid(int client, t_bids tbid, ErrorInfo error) {
		...
		tbid.invest_rate = backstageSet.investmentFee; // 投资管理费
...
/*		if(sms.status){
	...
	public void auditToFundraise(ErrorInfo error){
		...
		/* 3.修改状态 */
		String hql = "update t_bids set allocation_supervisor_id=?, audit_time=?, manager_fee=?, audit_suggest=?, status=? where id=? and user_id=? and status=?";

		Query query = JPA.em().createQuery(hql);
		query.setParameter(1, this.allocationSupervisorId);// 审核人
		query.setParameter(2, new Date());// 审核时间
		// 借款管理费率  add by xd.wang
		query.setParameter(3, this.managerFee);
		query.setParameter(4, this.auditSuggest);// 审核意见
		query.setParameter(5, Constants.BID_FUNDRAISE);// 审核状态为借款中
		query.setParameter(6, this.id); // 标ID
		query.setParameter(7, this.userId); // 用户ID
		query.setParameter(8, Constants.BID_AUDIT); // 当前状态必须为审核中
	...
	public void fundraiseToLoanNotThroughBC(ErrorInfo error) {
		...
		TemplateStation station = new TemplateStation();
		TemplateEmail email = new TemplateEmail();
		email.setId(Templets.E_FULL_FAIL);
		station.setId(Templets.M_FULL_FAIL);
	...
	public void eaitLoanToRepayment(ErrorInfo error){
		...
		/* 确定当前bid的状态 */
		//修改业务逻辑  还款中或放款中   add by xd.wang
		if(Constants.BID_EAIT_LOAN != this.status && Constants.BID_RELEASEING != this.status ){
			error.msg = "非法审核!";
	...
	public void doEaitLoanToRepayment(ErrorInfo error){
		...
		/* 修改状态 */
		String hql = "update t_bids set release_level=release_level+?, allocation_supervisor_id=?, audit_time=?, status=? where id=? and user_id=? and status=?";
		//放款状态  分批为true  add by xd.wang
		boolean  realeaseFlag = false;
		Query query = JPA.em().createQuery(hql);
		// 分批放款百分比 add by xd.wang
		query.setParameter(1, this.releaseLevel);
		query.setParameter(2, this.allocationSupervisorId);// 审核人
		query.setParameter(3, new Date());// 审核时间
		//分批放款标的状态处理 add by xd.wang
		if(this.releaseLevel< 1 && Constants.BID_RELEASEING != this.status){
			query.setParameter(4, Constants.BID_RELEASEING);// 非一次性放款，审核状态为放款中
			realeaseFlag = true;
		}else{
			query.setParameter(4, Constants.BID_REPAYMENT);// 一次放款，审核状态为还款中 
		}
		query.setParameter(5, this.id); // 标ID
		query.setParameter(6, this.userId); // 用户ID
		//分批放款标的状态处理 add by xd.wang
		if(this.releaseLevel< 1 && Constants.BID_EAIT_LOAN != this.status){
			query.setParameter(7, Constants.BID_RELEASEING); // 当前状态必须为放款中
		}else{
			query.setParameter(7, Constants.BID_EAIT_LOAN); // 当前状态必须为待放款
		...
		//全部放款后相应业务处理  add by xd.wang
		if(realeaseFlag==false){
		...
		/* 打钱给用户 */
		row = DealDetail.addUserFund(this.userId,Arith.mul(this.amount, this.releaseLevel) );
		...
		dealDetail = new DealDetail(this.userId, DealType.ADD_LOAN_FUND,
				Arith.mul(this.amount, this.releaseLevel), this.id, detail.get("user_amount"), detail.get("freeze"),
		...
		if(realeaseFlag==false){
			...
			dealDetail = new DealDetail(this.userId, DealType.CHARGE_LOAN_SERVER_FEE,
					...
					detail.get("receive_amount"), "支付借款服务费");
			...
			dealDetail.addPlatformDetail(DealType.LOAN_FEE, this.id, this.userId,
					-1, DealType.ACCOUNT, this.serviceFees, 1, "支付借服务理费",
					error);
		...
		//只要触发放款，就进行相应业务处理  add by xd.wang
		if(realeaseFlag==true || this.releaseLevel==1){
		...
		if(sms.status && StringUtils.isNotBlank(this.user.mobile)){
			...
			content = content.replace("money",DataUtil.formatString(Arith.mul(amount, this.releaseLevel) - serviceFees));//到账金额
			sms.addSmsTask(this.user.mobile, content);
		}
...
//	}
	...
	public long getUpId() {
		if(0 == this.upId) {
			...
			case 500: sql = sql3; sql += " and b.status in (-1, -2, -5, -6)"; break; //500.待付款投资账单列表。
			case 501: sql = sql3; sql += " and b.status in (-2)"; break; //501.逾期未付投资账单列表。
			case 502: sql = sql3; sql += " and b.status in (-4, 0)"; break; //502.已付款投资账单列表。
			case 503: sql = sql3; sql += " and b.status in (-3)"; break; //503.本金垫付投资账单列表。
	...
	public static List<FundraiseingBid> queryBid(int status,  ErrorInfo error) {
		error.clear();
		...
		if(count < 1)
			return new ArrayList<FundraiseingBid>();
