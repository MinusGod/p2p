...
public class Invest implements Serializable{
	...
	public static PageBean<v_invest_records> queryBidInvestRecords(int currPage, int pageSize,long bidId,ErrorInfo error){
		...
		sql.append(" order by time ,id desc");
	...
	public static PageBean<v_front_all_bids> queryAllBids(int showType, int currPage,int pageSize,String _apr,String _amount,String _loanSchedule,String _startDate,String _endDate,String _loanType,String minLevelStr,String maxLevelStr,String _orderType,String _keywords,ErrorInfo error){
		...
		if(StringUtils.isBlank(_apr) && StringUtils.isBlank(_amount) && StringUtils.isBlank(_loanSchedule) && StringUtils.isBlank(_startDate) && StringUtils.isBlank(_endDate) && StringUtils.isBlank(_loanType) && StringUtils.isBlank(minLevelStr) && StringUtils.isBlank(maxLevelStr) && StringUtils.isBlank(_orderType) &&  StringUtils.isBlank(_keywords)){
			...
			try {
				...
				params.add(showType);
				//添加按状态排序   add by xd.wang
				sql.append(" order by t_bids.status asc,t_bids.loan_schedule asc,t_bids.is_hot desc,t_bids.id desc");
				...
				e.printStackTrace();
				error.msg = "系统繁忙，给您带来的不便敬请谅解！";
				error.code = -1;
			}
			
			conditionMap.put("apr", 0);
			conditionMap.put("amount", 0);
			conditionMap.put("loanSchedule", 0);
			conditionMap.put("startDate", 0);
			conditionMap.put("endDate", 0);
			conditionMap.put("minLevel", 0);
			conditionMap.put("maxLevel", 0);
			conditionMap.put("orderType", 0);
			conditionMap.put("loanType", 0);
	...
	public static long queryTotalInvestCount(ErrorInfo error) {
		...
		try {
			...
			Logger.info("查询投资交易总数(债权转让)时，%s", e.getMessage());
			error.code = -1;
			error.msg = "查询投资交易总数失败";
		...
		try {
			...
			Logger.info("查询投资交易总数（投资）时，%s", e.getMessage());
			error.code = -1;
			error.msg = "查询投资交易总数失败";
...
//			robot.status = Constants.IPS_ENABLE ? false : true;
	...
	public static int closeUserBidRobot(long userId) {
		...
		try {
			...
			Logger.error("投资->查询开启自动投标状态:" + e.getMessage());
...
//            page.totalCount = QueryUtil.getQueryCountByCondition(em, sql.toString(), params);
	 ...
	 * @return
	 */
	public static void updateBidExpiretime(long bidId, double service_fees, ErrorInfo error){
		error.clear();
		...
		int rows = 0;
		try {
			rows = em.createQuery("update t_bids set real_invest_expire_time = ? where id=?")
					.setParameter(1, new Date())
					.setParameter(2, bidId)
					.executeUpdate();		
		} catch (Exception e) {
			e.printStackTrace();
			Logger.error("更新借款标满标时间时：%s", e.getMessage());
			error.code = -1;
			error.msg = "更新借款标满标时间失败";
	...
	public static Map<String,String> bidMap(long bidId,ErrorInfo error){
		...
		try {
			...
			e.printStackTrace();
			error.msg = "对不起！系统繁忙，给您造成的不便敬请谅解！";
			error.code = -11;
	...
	public static void invest(long userId, long bidId, int investTotal, String dealpwdStr, boolean isAuto, int client, ErrorInfo error) {
		...
		if( time> time2){
			...
			error.code = -2;
			JPA.setRollbackOnly();
			Logger.error("此借款标已经不处于招标状态，事务回滚");
		...
		if (error.code < 0) {
			...
			Logger.error("你的账户资金出现异常变动，事务回滚");
	...
	public static void doInvest(t_users user1, long bidId, double investTotal, String pMerBillNo, double pFee, int client, double award, double bid_fee, ErrorInfo error) {
		...
		if (amount == (hasInvestedAmount + investTotal)) {
			...
			// 更新满标时间
			updateBidExpiretime(bidId, bid_fee,  error);
			BackstageSet backstageSet = BackstageSet.getCurrentBackstageSet();
			String content=bid.get("title")+"("+bid.get("id")+")借款标已于"+DateUtil.LongToString()+"被投满，请及时到后台进行审核！";
			SMSUtil.sendSMS(backstageSet.auditBid, content, error);
		...
		if (error.code < 0) {
			JPA.setRollbackOnly();
			Logger.error("投标用户增加系统积分失败，事务回滚");
		...
		if (error.code < 0) {
			error.msg = "对不起！系统繁忙！请您重试或联系平台管理员！";
			error.code = -9;
			JPA.setRollbackOnly();
			...
			if(investUser != null && investUser.size() > 0) {
				for(Invest userInvest : investUser) {
					...
					if(user.is_invest_push) {
						...
						device = "{\"title\":\"投资满标提醒通知\",\"description\":\"你有一条新的投资满标\","+ device+"}";
...
//		Date date = DateUtil.strToDate(dateTime);
	...
	public static List<v_front_all_bids> queryBids(){
		...
		sql.append(SQLTempletes.SELECT);
		sql.append(SQLTempletes.V_FRONT_ALL_BIDS);
		//优化前台标的显示 （投标->待放款->还款中->已还款最新信息）modify by xd.wang
		sql.append("   and t_bids.status in( ?, ?, ?, ?) order by t_bids.status,t_bids.loan_schedule,t_bids.time desc,is_hot desc");
				
		
		List<Object> params = new ArrayList<Object>();
		//params.add(Constants.BID_ADVANCE_LOAN);
		params.add(Constants.BID_FUNDRAISE);
		params.add(Constants.BID_EAIT_LOAN);
	...
	public static List<v_invest_records> bidInvestRecord(
		...
		try {
			...
			Logger.error("投资->标投资记录,查询总记录数:" + e.getMessage());
	...
	public static List<v_bill_board> investBillboard(){
		...
		try {
            ...
            //限制投资风云榜 取3条记录 add by xd.wang
            query.setMaxResults(Constants.THREE);
	...
	public static void creatBidPactJob(){
		...
		for(Object bidId : bidIds){
			...
			try {
				...
				Invest.creatInvestPact(_bidId, error);//生成投资合同
			} catch (Exception e) {
				continue ;
			}
		}
