...
import payment.ips.util.IpsPayService;
import play.Logger;
...
import controllers.front.account.Spread;
...
//		}
	 ...
	 * @return
	 */
	public static PageBean<v_supervisors> queryCustomers(int currPage, int pageSize, String keyword, ErrorInfo error) {
		error.clear();
		...
		if (StringUtils.isNotBlank(keyword)) {
			sql.append(" and name like ? or reality_name like ?");
			params.add("%" + keyword + "%");
			params.add("%" + keyword + "%");
		}
	 ...
	 * @return
	 */
	public static PageBean<v_customers> queryCustomers2(int currPage, int pageSize, String keyword, int isExport, ErrorInfo error) {
		error.clear();
		...
		StringBuffer sql = new StringBuffer(v_customers.SQL);
		
		StringBuffer sqlCount = new StringBuffer("select count(1) from t_supervisors susr where is_customer = 1");
		
		if (StringUtils.isNotBlank(keyword)) {
			sql.append(" and susr.name like ?");
			sqlCount.append(" and susr.name like ?");
		}

		Object count = null;
		List<v_customers> page = null;
		EntityManager em = JPA.em();
 
		try {
			
            Query query = em.createNativeQuery(sql.toString(), v_customers.class);
		...
		PageBean<v_customers> bean = new PageBean<v_customers>();
		bean.pageSize = pageSize;
		bean.currPage = currPage;
		bean.totalCount = count==null?0:Integer.parseInt(count.toString());
		bean.page = page;
		bean.conditions = map;
...
//		params.add(Constants.CUSTOMER_RIGHT_ID);
	 ...
	 * @return
	 */
	public static PageBean<v_supervisors> queryCandidateSpreaders(int currPage,
			int pageSize, String keyword, ErrorInfo error) {
		error.clear();
		...
		StringBuffer sqlCount = new StringBuffer("select count(1) from t_supervisors where 1=1 ");
		...
		if (StringUtils.isNotBlank(keyword)) {
			sql.append(" and (name like ? or reality_name like ? or mobile1 like ? or email like ?)");
			sqlCount.append(" and (name like ? or reality_name like ? or mobile1 like ? or email like ?)");
			params.add("%" + keyword + "%");
			params.add("%" + keyword + "%");
			params.add("%" + keyword + "%");
			params.add("%" + keyword + "%");
		...
		//直客不在已有的直客当中
		sql.append(" and not EXISTS (select 1 from t_spreaders spr where spr.supervisor_id = t_supervisors.id)");
		sqlCount.append(" and not EXISTS (select 1 from t_spreaders spr where spr.supervisor_id = t_supervisors.id)");
		//管理员所在权限组要有该菜单权限  
		//sql.append(" and EXISTS ( SELECT 1 from t_rights_of_supervisor rsup where rsup.supervisor_id = t_supervisors.id and rsup.right_id = 125)");
		//sqlCount.append(" and EXISTS ( SELECT 1 from t_rights_of_supervisor rsup where rsup.supervisor_id = t_supervisors.id and rsup.right_id = 125)");
