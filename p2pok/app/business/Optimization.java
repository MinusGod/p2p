...
public class Optimization {
	...
	public static class UserOZ implements Serializable{
        ...
        public int receivableInvestBidsCount; // 收款中投资标
        public int untreatedBillsCount; // 未处理借款账单
        public int untreatedInvestBillsCount; // 未处理投资账单
	...
	public static class BidOZ {
    	 ...
    	 * @return
    	 */
    	public static List<QualityBid> queryQualityBid2(int size, ErrorInfo error){
    		String sql = "SELECT `b`.`id` AS `id`,`b`.`user_id` AS `user_id`,`b`.`image_filename` AS `bid_image_filename`,`p`.`small_image_filename` AS `small_image_filename`,`b`.`title` AS `title`,`b`.`loan_schedule` AS `loan_schedule`,`b`.`apr` AS `apr`,`b`.`amount` AS `amount`,`b`.`period_unit` AS `period_unit`,`b`.`period` AS `period`,`b`.`has_invested_amount` AS `has_invested_amount`,`r`.`name` AS `repayment_type_name` FROM ((`t_bids` `b` LEFT JOIN `t_products` `p` ON ((`b`.`product_id` = `p`.`id`))) LEFT JOIN `t_dict_bid_repayment_types` `r` ON (( `r`.`id` = `b`.`repayment_type_id`))) WHERE b.amount > b.has_invested_amount AND b. STATUS IN (?, ?) ORDER BY `b`.`id` DESC";
		...
		public static List<QualityBid> queryBidsFive(ErrorInfo error) {
			String sql = "SELECT `b`.`id` AS `id`,`b`.`user_id` AS `user_id`,`b`.`image_filename` AS `bid_image_filename`,"
					+ "`p`.`small_image_filename` AS `small_image_filename`,"
					+ "`b`.`title` AS `title`,`b`.`loan_schedule` AS `loan_schedule`,`b`.`apr` AS `apr`,`b`.`amount` AS `amount`,"
					+ "`b`.`period_unit` AS `period_unit`,`b`.`period` AS `period`,`b`.`has_invested_amount` "
					+ "AS `has_invested_amount`,`r`.`name` AS `repayment_type_name` "
					+ "FROM ((`t_bids` `b` LEFT JOIN `t_products` `p` ON ((`b`.`product_id` = `p`.`id`))) "
					+ "LEFT JOIN `t_dict_bid_repayment_types` `r` ON (( `r`.`id` = `b`.`repayment_type_id`))) "
					+ "WHERE  b. STATUS IN (?, ?, ?, ?, ?, ?) ORDER BY `b`.`status`,`b`.`loan_schedule`,`b`.`time` DESC,`b`.`is_hot` DESC";
			                                      
    		EntityManager em = JPA.em();
			Query query = em.createNativeQuery(sql, QualityBid.class);
			query.setParameter(1, Constants.BID_ADVANCE_LOAN);
			query.setParameter(2, Constants.BID_FUNDRAISE);
			query.setParameter(3, Constants.BID_EAIT_LOAN);
			query.setParameter(4, Constants.BID_REPAYMENT);
			query.setParameter(5, Constants.BID_REPAYMENTS);
			query.setParameter(6, Constants.BID_COMPENSATE_REPAYMENT);
	...
	public static class InvestOZ {
        ...
        public static void add(){
            ...
            try {
                ...
                Logger.error("添加投资子账户投资情况统计表时：" + e.getMessage());
            }
        }
	}
