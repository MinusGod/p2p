...
public class ServiceFee {
	...
	public static double loanServiceFee(double amount, int period, int unit, ErrorInfo error) {
	    ...
	    /** 公式：按借款本金 * ?% /360 * 借款天数 **/
			num4 = new BigDecimal(num4).compareTo(new BigDecimal(Constants.BORROW_MANAGE_MAXRATE))>0?Constants.BORROW_MANAGE_MAXRATE:num4;	
		...
		double borrowManageFeeFiftyPercent = Arith.mul(amount, Arith.div(Constants.BORROW_MANAGE_MAXRATE, 100, 10));
		div = new BigDecimal(div).compareTo(new BigDecimal(borrowManageFeeFiftyPercent))>0 ? borrowManageFeeFiftyPercent : div;
...
/**
 * 计算管理费合计  add by xd.wang
 * @param manager_fee 
 * @param manager_amount
 * @param period_unit
 * @param period
 * @param repayment_type_id 
 * @return
 */
	public static double managerAmountCompute(double amount,
			double manager_fee, Integer unit, Integer period, Integer repayment) {
			if(0 == manager_fee || 0 == period || repayment !=Constants.PAID_MONTH_ONCE_REPAYMENT || 
				unit < -1 || unit > 1)
				return 0;
			//通过借款管理费年利率得到月利率 
			double monthRate = Double.valueOf(manager_fee * 0.01)/12.0;
			double managerAmountSum = 0;
			//只当支付方式为先息后本才计算管理费合计，否则默认0，业务待优化   add by xd.wang
			if(unit == Constants.DAY){//秒还还款和天标的总利息
				managerAmountSum = 0;
				...
				if(repayment == Constants.PAID_MONTH_EQUAL_PRINCIPAL_INTEREST){
					...
					managerAmountSum = Arith.round(Arith.mul(Arith.mul(amount, monthRate), period), 2);
					
				}
			}
			
			return Arith.round(managerAmountSum, 2);
		}
}
