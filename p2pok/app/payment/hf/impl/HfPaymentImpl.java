...
public class HfPaymentImpl implements PaymentInterface {
	...
	public Map<String, Object> register(ErrorInfo error, int client, Object... obj) {
		...
		LinkedHashMap<String, String> paramMap = hfPaymentService.register(user.mobile,
				user.email, orderNo,client,user.idNumber,user.realityName);
	...
	public Map<String, Object> bidCreate(ErrorInfo error, int client, Object... obj) {
		...
		//标的发布成功，页面跳转，提示完善资料
		HfPaymentCallBackAction.getInstance().addBidInfoWS(error, bid,client);
	...
	public Map<String, Object> offlineRepayment(ErrorInfo error, int client, Object... obj) {
		...
		for(Map<String,Object> billInvestMap : billInvestList){
			...
			String subOrdId = String.valueOf(billInvestMap.get("mer_bill_no"));
			t_invests invest = new t_invests();
			invest = invest.findById((Long) billInvestMap.get("invest_id"));
			String subOrdDate = DateUtil.simple(invest.time);
			String principalAmt = String.valueOf(HfPaymentUtil.formatAmount((Double) billInvestMap.get("receive_corpus")));
			String interestAmt = String.valueOf(HfPaymentUtil.formatAmount((Double) billInvestMap.get("receive_interest")));
			String dzObject = user.ipsAcctNo;
			
			// 参数组装
			LinkedHashMap<String, String> paramMap = 
			          hfPaymentService.repaymentHelp( bid.bid_no, orderNo,  subOrdId,  subOrdDate,  principalAmt, 
			           interestAmt,  dzObject,  inCustId);
	...
	public Map<String, Object> agentRecharge(ErrorInfo error, int client, Object... obj) {
		...
		hfPaymentService.printRequestData(paramMap, "转账提交参数", PayType.AGENTRECHARGE);	
	...
	@Override
	public Map<String, Object> agentRechargeAdj(ErrorInfo error, int client, Object... obj) {
		String ipsAcctNo = obj[0].toString();  //资金托管账号
		double amount = (Double)obj[1];  //转账金额
		String merOrderNo = obj[2].toString();  //转账流水号
		//long agentOrderNo = Long.parseLong(obj[3].toString());  //用户支付流水号
		...
		//参数组装
		LinkedHashMap<String, String> paramMap = hfPaymentService.doTransferAdj(merOrderNo, amount, ipsAcctNo);
	    ...
	    hfPaymentCallBackService.transferhbAdj(resultMap, "转账响应参数", error);
	...
	public Map<String, Object> bidAuditSucc(ErrorInfo error, int client, Object... obj) {
		...
		for(t_invests invests : investList){
			//按百分比放款金额  add by xd.wang
			double firstReleaseAmount = Arith.mul(invests.amount, bid.releaseLevel);
			//订单号
			String ordId = hfPaymentService.createBillNo();
			//投资人第三方唯一标示
			...
			LinkedHashMap<String, String> paramMaps = hfPaymentService.bidAuditSucc(ordId, outCustId, firstReleaseAmount, invests.bid_fee, subOrdId, subOrdDate, inCustId, unFreezeOrdId, freezeOrdId, divDetails, reqExt, bid.bidNo);
	...
	public Map<String, Object> repayment(ErrorInfo error, int client, Object... obj) {
		...
		double managementRate = Bid.queryInvestRate(bid.id);		
 		...
 		// 拼接接口所需的JSON格式投资账单列表
 		String inDetails = hfPaymentService.generateBatchRepaymentInDetails(list, managementRate,bill.managerAmount);
 		// 获取出账商户号
		...
		if(client == Constants.APP){
			//签名，状态码，仿重单处理;
			HfPaymentUtil.checkSign(resultMap, "批量还款同步回调", PayType.REPAYMENT, error);
			Map map = new HashMap();
			if(error.code > 0){
				bill.repayment(bid.userId, error);
				if(error.code==0){
					map.put("error", "-1");
					map.put("msg","还款成功");
					return map;
	...
	public Map<String, Object> withdraw(ErrorInfo error, int client, Object... obj) {
		...
		String orderNo = hfPaymentService.createBillNo();
		String usrCustId = user.ipsAcctNo;
		Map<String, Object> bankMap = PaymentProxy.getInstance().queryBindedBankCard(error, Constants.PC, user.ipsAcctNo);
		if(bankMap != null && StringUtils.isBlank(bankMap.get("result").toString())){
			return userBindCard(error, client, usrCustId);
		}
