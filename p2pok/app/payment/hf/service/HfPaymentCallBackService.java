...
public class HfPaymentCallBackService extends HfPaymentService{
	...
	public void withdraw(Map<String, String> paramMap, String desc, ErrorInfo error){
		...
		//提现对账处理
		if(paramMap.containsKey("RespType") && "Cash".equals(paramMap.get("RespType"))){  
	...
	/*	error = doCashAudit(paramMap,error);
		if(error.code < 0){
			return ;
		}
		*/
		...
		User.withdrawalNotice(userId, serviceFee+feeAmt, withdrawalId, "1,2", chargeMode, true, error);
	}
	 ...
	 * @param error
	 */
	public void withdraw0(Map<String, String> paramMap, String desc, ErrorInfo error){
	...
	/*	error = doCashAudit(paramMap,error);
		...
		*/
		...
		String orderNo = paramMap.get("MerPriv");
		Map<String,String> map=HFServiceFee.getServiceFee(paramMap);
		double serviceFee = Convert.strToDouble(map.get("ServFee") + "", 0.00);  //商户收取用户的服务费
		@SuppressWarnings("unused")
		double feeAmt = Convert.strToDouble(map.get("FeeAmt") + "", 0.00);  //手续费金额（汇付收），商户垫付
	 ...
	 * @return
	 */
	public static ErrorInfo doCashAudit(Map<String,String> maps,ErrorInfo error){
		if(!HfConstants.CASHAUDIT){
			error.code = 1;
			error.msg = "客户未开通提现复核业务";
			return error;
		}
		String ordId = maps.get("OrdId");
		String usrCustId = maps.get("UsrCustId");
		String transAmt = maps.get("TransAmt");
		String auditFlag = HfConstants.CASHAUDIT_S;
		ChinaPnrReqModel model = ChinaPnrReqModel.buildCashAuditParams(ordId, usrCustId, transAmt, auditFlag);
		String result = doCashAudit(model);
		JsonObject json = new JsonParser().parse(result).getAsJsonObject();
		//日志打印
		Logger.info("--------提现复核-----"+json.toString());
		String respCode = json.get("RespCode").getAsString();
	 ...
	 * @return
	 */
	public static String doCashAudit(ChinaPnrReqModel model){
		LinkedHashMap<String, String> map = new LinkedHashMap<String, String>();
		map.put("Version", model.getVersion());
		map.put("CmdId", model.getCmdId());
		map.put("MerCustId", model.getMerCustId());
		map.put("OrdId", model.getOrdId());
		map.put("UsrCustId", model.getUsrCustId());
		map.put("TransAmt", model.getTransAmt());
		map.put("AuditFlag", model.getAuditFlag());
		map.put("BgRetUrl", BaseController.getBaseURL() + model.getBgRetUrl());
		
		String result = HfPaymentUtil.postMethod(HfConstants.CHINAPNR_URL, map, "utf-8");
		return result;
	}
	
	public static void cashAuditSucc(){
	 ...
	 * @param error 
	 */
	public void transferhb(Map<String, String> paramMap, String desc, ErrorInfo error) {
		error.clear();
		...
		String ipsAcctNo = maps.get("InCustId");
		String amount = maps.get("TransAmt");
        String OrdId = maps.get("OrdId");
		User user = new User();
		user.findUserByipsAcctNo(ipsAcctNo);
		DataSafety data = new DataSafety();
		data.id = user.id;
		if (!data.signCheck(error)) {
			return;
		}
		t_user_agent_pay agent =
		    AgentPayment.queryAgentPayByAgentNo(Long.valueOf(OrdId));
		if (agent.status != 0) {
			error.clear();
			error.msg = "已经转账成功!";
			return;
		...
		//  增加用户资金
		DealDetail.addUserFund(user.id, Double.valueOf(amount));  
		...
		DealDetail dd = new DealDetail(user.id, DealType.RECHARGE_USER, Double.valueOf(amount), -1l, forDetail.user_amount, forDetail.freeze,
				forDetail.receive_amount, "现金红包");
		...
		//平台支出，转账金额
		DealDetail.addPlatformDetail(DealType.AGENT_RECHARGE, Long.valueOf(OrdId), -1, user.id , DealType.ACCOUNT, Double.valueOf(amount), 2, "商户转账,红包发放", error);
		...
		// 插入用户事件
		DealDetail.userEvent(user.id, UserEvent.RECHARGE, "现金红包", error);
		if (error.code < 0) {
			return;
		}
		
		AgentPayment.updateAgentRechargeStatus(Long.valueOf(OrdId), 0, 1, error);
	 ...
	 * @param error 
	 */
	public void transferhbAdj(Map<String, String> paramMap, String desc, ErrorInfo error) {
		error.clear();
	...
	/*	v_user_for_details forDetail = DealDetail.queryUserBalance(user.id, error);
		...
		//平台支出，转账金额
		DealDetail.addPlatformDetail(DealType.AGENT_RECHARGE, Long.valueOf(OrdId), -1, user.id , DealType.ACCOUNT, Double.valueOf(amount), 2, "商户转账,平账", error);
	...
	*/
	...
	public void modifyBillInvestMerBillId(long id,String mer_bill_no,ErrorInfo error){
		...
		try{
			...
			if(row <1){
				error.code = -1;
				error.msg = "更新投资人账单流水号-用于本金垫付异常";
				Logger.error("###10###更新投资人账单流水号-用于本金垫付时影响行数:%s", row);
			}
			JPAUtil.transactionCommit();
		}catch(Exception e){
			error.code = -1;
			error.msg = "更新投资人账单流水号-用于本金垫付异常";
			Logger.error("###10###更新投资人账单流水号-用于本金垫付时:%s", e.getMessage());
		}
	}
