...
public class ChinaPnrReqModel implements Serializable { 
	 ...
	 * @return
	 */
	public static ChinaPnrReqModel buildCashAuditParams(String ordId,String usrCustId,String transAmt,String auditFlag){
		ChinaPnrReqModel model = new ChinaPnrReqModel();
		model.setVersion(HfConstants.VERSION1);
		model.setCmdId(HfConstants.CMD_CASHAUDIT);
		model.setMerCustId(HfConstants.MERCUSTID);
		model.setOrdId(ordId);
		model.setUsrCustId(usrCustId);
		model.setTransAmt(transAmt);
		model.setAuditFlag(auditFlag);
		model.setBgRetUrl("payment/chinapnr/cashAudit");
		return model;
	}
	...
	public Map<String,String> converMap(){
		...
		try {
			maps = BeanMapUtils.toMap(this);
			if(this.getBgRetUrl()!=null && !"".equals(this.getBgRetUrl())){
				maps.put("BgRetUrl", BaseController.getBaseURL() + this.getBgRetUrl());
			}
			if(this.getRetUrl()!=null && !"".equals(this.getRetUrl())){
				maps.put("RetUrl", BaseController.getBaseURL() + this.getRetUrl());
			}
