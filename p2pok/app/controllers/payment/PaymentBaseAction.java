...
public class PaymentBaseAction extends BaseController{
	...
	public static void payErrorInfo(int code, String msg, PayType payType){
		...
		//微信提现、充值提示页面 add by xd.wang
		if(payType == payType.RECHARGEWX || payType == payType.WITHDRAWWX ){
			renderTemplate("wechat/others/popup/payErrorInfoWX.html",error);
		}
		//微信投标 提示页面 add by xd.wang
		else if(payType == payType.INVESTWX){
			renderTemplate("wechat/others/popup/bidPop.html",error);
		}
		render(error);
	}
	...
	public static void approve() {
		...
		else if("YB".equalsIgnoreCase(payType)){
			src += Constants.YEE_LOGO_IMAGE;
		}
