...
public class FyPaymentAction extends PaymentBaseAction implements PaymentInterface{
	...
	public Map<String, Object> bidAuditFail(ErrorInfo error, int client, Object... obj) {
		...
		//修复审核意见没有入库的bug begin
		if(obj.length == 4){
			bid.auditSuggest = (String)obj[2]; // 审核意见
			bid.allocationSupervisorId = (Long)obj[3]; // 审核人
		}
		//end
		//初始化借款人
		User borrower = new User();
	...
	public Map<String, Object> invest(ErrorInfo error, int client, Object... obj) {
		...
		try {
			...
			if(error.code < 0){
				payErrorInfo(ErrorInfo.createError(error, error.code, error.msg, "/front/invest/investHome", "我要投资页面!"));
				return null;
			}
		} catch (Exception e) {
			if(error.code < 0){
	...
	@Override
	public Map<String, Object> agentRechargeAdj(ErrorInfo error, int client,
			Object... obj) {
		// TODO Auto-generated method stub
		return null;
	}
