...
//			return "正版校验失败";
//		}
		System.out.println("$$$$$$$$$$$$$"+parameters.get("OPT"));
		...
		case AppConstants.APP_INVEST_BILLS: 
			...
			Logger.error("查询投资账单时：%s：", e.getMessage()); 
			} 
			break; 
		...
		case AppConstants.APP_LOANING_INVEST_BIDS:
			...
			Logger.error("查询等待满标的投资标时：%s：", e.getMessage());
			}
			break;
		case AppConstants.APP_RECEVING_INVEST_BIDS:
			...
			Logger.error("查询收款中的投资标列表时：%s：", e.getMessage());
			}
			break;
		...
		case AppConstants.APP_SUCCESS_DEBT:
			...
			Logger.error("查询已成功的投资标时：%s：", e.getMessage());
			}
			break;
		...
		case AppConstants.APP_INVEST_STATISTICS:
			...
			Logger.error("查询投资情况统计表时：%s：", e.getMessage());
			}
			break;
		...
		case AppConstants.APP_PLATFORM_INFO:
			...
			break;
			//开户操作[页面跳转至汇付]
		case AppConstants.APP_UNAME_PHONE_VERIFY:
			try {
				result = RequestData.verifyUnamePhone(parameters);
			} catch (Exception e) {
				Logger.error("用户注册 ：", e);
			}
			break;
		case AppConstants.APP_FIND_PWD_BY_SMS_T:
			try {
				result = RequestData.findPwdBySms(parameters);
			} catch (IOException e) {
				Logger.error("根据短信找回密码时：%s：", e.getMessage());
			...
			break;
		case AppConstants.APP_CREATE_ACCOUNT:
			try{
				result = RequestData.createAcct(parameters);
			} catch(Exception e) {
				Logger.error("开通资金托管账号时%s", e.getMessage());
			}
			break;
			
			//APP查询银行列表
		case AppConstants.APP_RECHARGE_BANKLIST:
			try{
				result = RequestData.rechargeBankList(parameters);
			} catch(Exception e) {
				Logger.error("查询用户开户信息时%s", e.getMessage());
			}
			break;
		...
		case AppConstants.APP_RECHARGE:
			...
			//查询用户开户信息
		case AppConstants.APP_QUERY_CREATE_INFO:
			try{
				result = RequestData.queryCreateInfo(parameters);
			} catch(Exception e) {
				Logger.error("查询用户开户信息时%s", e.getMessage());
			}
			break;
			
		case AppConstants.APP_CAPITAL_PROTECTION_PHOTO:
			try {
				result = RequestData.queryCapitalProtectionPage(parameters);
			} catch (Exception e) {
				Logger.error("查询本金保障页面", e);
			}
			break;
		case AppConstants.APP_QUEST_INFO:
			try {
				result = RequestData.bidQuest(parameters);
			} catch (Exception e) {
				Logger.error("标的回复%s", e);
			}
			break;
		}
