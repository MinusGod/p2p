...
public class RegistAndLogin extends BaseController {
	...
	public static void logining() {
		...
		}else if(RegexUtils.isMobileNum(userName)){
	 ...
	 * 跳转到注册页面
	 */
	public static void register(String un) {
		/**
	...
	public static void appregister(String un) {
		/**
		 * pc端 String loginOrRegister = Constants.LOGIN_AREAL_FLAG;
		 * 
		 * ErrorInfo error = new ErrorInfo(); String content =
...
//		String randomID = (String) Cache.get(params.get("randomID"));
		...
		flash.put("code", code);
		flash.put("recommendUserName", recommendName);
...
//		}
		...
		user.setCurrUser(user);
		renderTemplate("wechat/others/popup/registerPop.html",error);
		//render(user);
	}
...
//		}
	...
	/**
	 * 重写托管开户  add by xd.wang
	 */
	public static void createAcctWX() {
		ErrorInfo error = new ErrorInfo();
		PaymentProxy.getInstance().register(error, Constants.WECHAT, User.currUser());
//		flash.error(error.msg);
//		CheckAction.trustAccount();
	 ...
	 * 托管开户
	 */
	public static void createAcct(ErrorInfo error) {
		error.clear();
		PaymentProxy.getInstance().register(error, Constants.WECHAT, User.currUser());
//		flash.error(error.msg);
//		CheckAction.trustAccount();
