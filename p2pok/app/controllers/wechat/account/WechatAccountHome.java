...
public class WechatAccountHome extends BaseController {
	...
	public static void accountSign(){
		User user=User.currUser();
		String randomCode=SMSUtil.getRandom();
		String mobile=user.mobile;
		String userName=user.name;
		JSONObject json = new JSONObject();
		String url="http://rongyoujiaoyu.com:8080/SAWARD/save/";
		ErrorInfo error=new ErrorInfo();
		String responseCode=HttpClientHelp.sendAward(url, userName, randomCode, mobile);
		if("Fail".equals(responseCode)){
			json.put("msg", "请勿重复签到");
			renderJSON(json);
		}		
		SMSUtil.sendSMS(mobile,"您的随机码是:"+randomCode,error);
		if(error.code!=1){
			json.put("msg", "随机码发送失败");
			renderJSON(json);
		}
		json.put("msg", "签到成功");
	    renderJSON(json);
	...
	ErrorInfo error = new ErrorInfo();
		...
		if(StringUtils.isBlank(user.ipsAcctNo)){
			RegistAndLogin.createAcct(error);
		}
...
//			String bankCode = params.get("bankCode");
			...
			if (money < rechargeLowest || money > rechargeHighest) {
				flash.error("充值金额范围需在["+rechargeLowest+"~" + rechargeHighest+ "]之间");
				recharge();
			}
...
//				recharge();
//			}
			PaymentProxy.getInstance().recharge(error, Constants.WECHAT, money);
...
//		String endTime = params.get("endDate");
	...
	public static void submitRepayment(String payPassword, double amount, String billId){
		...
		if(Constants.IPS_ENABLE) {		
			...
			PaymentProxy.getInstance().repayment(error, Constants.WECHAT, bill, user.id);
	...
	public static void acceptDebts(String sign, String dealpwd){
		...
		if (Constants.IPS_ENABLE) {
			...
			//定向转让调用托管接口
			PaymentProxy.getInstance().debtorTransfer(error, Constants.WECHAT, debtId, dealpwd);
			
			receivedDebts(Constants.WECHAT_CURRPAGE, Constants.WECHAT_CURRPAGE, "0", "", 0, null);
		}
	...
	public static void transact(String sign, String dealpwd){
		...
		if(error.code < 0){
			render("@wechat.account.WechatAccountHome.receivedDebts");
		}
		...
		if (Constants.IPS_ENABLE) {
			...
			if(status != Constants.DIRECTIONAL_MODE && status != Constants.AUCTION_MODE && status != Constants.WAIT_CONFIRM){
				WechatAccountHome.receivedDebts(1, 10, null, null, 0, null);
			}
	...
	public static void home() {
		...
		List<t_content_advertisements> homeAds = Ads.queryAdsByLocation(Constants.HOME_PAGE_WECHAT, error); // 广告条
		if (error.code < 0) {
			errorShow(error.msg);
		}
	...
	public static void userBindCard() {		
		ErrorInfo error = new ErrorInfo();
		User user = User.currUser();
		PaymentProxy.getInstance().userBindCard(error, Constants.WECHAT, user.ipsAcctNo);
	}
}
