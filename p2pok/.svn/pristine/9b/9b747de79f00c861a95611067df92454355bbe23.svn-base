...
public class LoanUsers extends SupervisorController {
	...
	public static void loanUserManager(){
		...
		int userCount = 0;
		int beRepaymenting_bill_count=0;
		int repayed_bill_count=0;
		int overdue_bill_count=0;
		int bad_bid_count=0;
		if(page.page!=null){//有问题，直接使用查询语句查
			userCount = page.page.size();
			for(v_user_loan_info_bill bill :page.page){
				
				beRepaymenting_bill_count += bill.beRepaymenting_bill_count;
				repayed_bill_count += bill.repayed_bill_count;
				overdue_bill_count += bill.overdue_bill_count;
				bad_bid_count += bill.bad_bid_count;
			}
		}
		
		render(page,userCount,beRepaymenting_bill_count,repayed_bill_count,overdue_bill_count,bad_bid_count);
	}
	...
	public static void deptLoanUserManager(int isExport){
		...
		page = User.queryUserInfoBillD(isExport==Constants.IS_EXPORT?Constants.NO_PAGE:0, name, currPageStr, pageSizeStr, error);
		...
		if(isExport == Constants.IS_EXPORT){
			...
			File file = ExcelUtils.export("已分配的借款会员列表",
			arrList,
			new String[] {"借款人", "待还账单期数", "待还账单金额", "本月将到期期数", "本月将到期金额 ", "账户可用余额","客服"},
			new String[] {"name", "unpaid_bill_count", "unpaid_bill_amount","maturity_bill_count","maturity_bill_amount","balance", "supervisor_name"});
			   
			renderBinary(file, "已分配的借款会员列表.xls");
		}
	...
	public static void queryAllSupervisors(int currPage, int pageSize, String keyword, String userId, String type, String bidId){
		...
		page = Supervisor.queryCustomers(currPage, pageSize, keyword, error);
