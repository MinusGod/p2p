...
public class UserAuditItem  implements Serializable{
	...
	public static List<UserAuditItem> queryUserAllAuditItem(long userId, String mark){
		...
		List<ProductAuditItem> pitems = ProductAuditItem.queryAuditByProductMark(mark, true, true);
	 ...
	 * @return
	 */
	public static List<UserAuditItem> queryUserAuditedItem(long userId, String mark){
		List<UserAuditItem> items = new ArrayList<UserAuditItem>();
		List<t_user_audit_items> tuItems = null;
		String hql = "select new t_user_audit_items(audit_item_id, mark) from t_user_audit_items where user_id = ? and status = 2 group by audit_item_id";
	...
	public static void checkAuditItemIsExpired(){
        ...
        String hql = "select distinct b1.user_id from t_bids  b1 where b1.last_repay_time is not null AND b1.user_id  not in (select distinct b2.user_id from t_bids b2 where b2.last_repay_time is  null and b2.status >= 0) ";
        Query query0 = JPA.em().createQuery(hql);
        ids=query0.getResultList();	
		...
		hql = "update t_user_audit_items set status = ? where status=2 and user_id in (" + strId + ")";
	...
	public void createUserAuditItem(ErrorInfo error) {
		...
		for (String image : names) {
			...
			item.image_file_name = image.substring(0, image.indexOf(".")==-1 ? image.length() : image.indexOf("."));	//去掉多余的后缀
