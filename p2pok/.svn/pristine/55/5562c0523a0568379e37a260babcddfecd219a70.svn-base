...
public class BillInvests implements Serializable{
	...
	//add by xd.wang
	public double receiveManagerAmount;
	public double realReceiveManagerAmount;
	...
	public void setId(long id){
		...
		//add by xd.wang
		this.receiveManagerAmount = invest.receive_manager_amount;
		this.realReceiveManagerAmount = invest.real_receive_manager_amount;
		bid = new Bid();
	...
	public static List<v_bill_invest> queryMyAllInvestBills(ErrorInfo error) {
		...
		try {
			...
			e.printStackTrace();
			Logger.error("查询我所有的投资账单:" + e.getMessage());
			error.code = -1;
			error.msg = "查询我所有的投资账单失败!";
	...
	/**
 	...
 	public static PageBean<v_bill_invest> queryMyInvestBills(int payType, int isOverType,
		...
		try {
			...
			e.printStackTrace();
			Logger.info("查询我的投资账单时："+e.getMessage());
			error.code = -1;
			error.msg = "由于数据库异常，查询我的投资账单失败";
 	...
 	public static v_bill_invest_detail queryMyInvestBillDetails(long id, long userId, ErrorInfo error){
		...
		sql.append(" and a.id = ? and a.user_id = ?");
 		...
 		try {
			...
			e.printStackTrace();
			Logger.info("查询我的投资账单详情时："+e.getMessage());
			error.code = -1;
			error.msg = "由于数据库异常，查询我的投资账单详情失败";
 	...
 	public static PageBean<t_bill_invests> queryMyInvestBillReceivables(long bidId,long userId, long investId, int currPage, int pageSize, ErrorInfo error){
		...
		try {
			...
			e.printStackTrace();
			Logger.info("查询我的投资账单收款情况时："+e.getMessage());
			error.code = -1;
			error.msg = "由于数据库异常，查询我的投资账单收款情况失败";
 	...
 	public static double getInvestManagerFee(double receiveInterest, double managementRate, long investUserId){
		...
		//被直客推广获得投资优惠
		t_spread_users spreader = t_spread_users.find("user_id = ?", investUserId).first();
		
		double inviteRate = invite==null ? 100.0 : invite.invited_user_discount;
		double spreadRate = spreader==null ? 100.0 : spreader.discount_rate;
		
		manageFee = manageFee * (inviteRate / 100) * (spreadRate / 100);	//只会获得一个优惠
 	...
 	public static List findBillInvestsByBidIdAndPeriods(long bidId, int periods) {
 		...
 		String sql = " select new Map(invest.id as id, invest.invest_id as invest_id, invest.mer_bill_no as mer_bill_no, invest.receive_interest as bid_fee, invest.invest_id as investId, "
 		...
 		try {
			...
			Logger.error("------- 获取投资账单列表失败：" +  e.getMessage());
			e.printStackTrace();
			return null;
		}
