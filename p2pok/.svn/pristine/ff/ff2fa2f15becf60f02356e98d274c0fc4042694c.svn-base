...
public class StatisticDebt {
	 ...
	 public static long queryIncreaseDebtAccount(int year,int month,ErrorInfo error){
		 error.clear();
...
//		String sql = "SELECT COUNT(DISTINCT b.bid_id) FROM t_invests b WHERE b.id IN (select a.invest_id from t_invest_transfers a where DATE_FORMAT(a.time, '%Y%m') = DATE_FORMAT(CURDATE(),'%Y%m') AND DATE_FORMAT(a.start_time, '%Y%m') = DATE_FORMAT(CURDATE(),'%Y%m') AND a.`status` NOT IN (?, ?))";
		String sql = "SELECT COUNT(DISTINCT b.bid_id) FROM t_invests b WHERE b.id IN (select a.invest_id from t_invest_transfers a where (YEAR(a.time)=? AND MONTH(a.time)=?) AND (YEAR(a.start_time)=? AND MONTH(a.start_time)=?) AND a.`status` NOT IN (?, ?))";
		Object obj;
		...
		Query query = JPA.em().createNativeQuery(sql)
					...
					.setParameter(5, Constants.DEBT_NOPASS)
					.setParameter(6, Constants.DEBT_AUDITING);
	 ...
	 public static Double queryIncreaseDebtAmountSum(int year,int month,ErrorInfo error){
		 error.clear();
...
//		String sql = "select SUM(a.debt_amount) from t_invest_transfers a where DATE_FORMAT(a.time, '%Y%m') = DATE_FORMAT(CURDATE(),'%Y%m') AND DATE_FORMAT(a.start_time, '%Y%m') = DATE_FORMAT(CURDATE(),'%Y%m') AND a.`status` NOT IN (?, ?)";
		String sql = "select SUM(a.debt_amount) from t_invest_transfers a where (YEAR(a.time)=? AND MONTH(a.time)=?) AND (YEAR(a.start_time)=? AND MONTH(a.start_time)=?) AND a.`status` NOT IN (?, ?)";
		Query query = JPA.em().createNativeQuery(sql)
				.setParameter(1, year)
				.setParameter(2, month)
				.setParameter(3, year)
				.setParameter(4, month)
				.setParameter(5, Constants.DEBT_NOPASS).setParameter(6, Constants.DEBT_AUDITING);
	 ...
	 public static Long queryHasOverdueDebt(int year,int month,ErrorInfo error){
		error.clear();
...
//		String sql = "SELECT COUNT(DISTINCT a.bid_id) FROM t_bills a WHERE a.overdue_mark <> ? AND DATE_FORMAT(a.mark_overdue_time,'%Y%m') <= DATE_FORMAT(CURDATE(),'%Y%m') AND a.bid_id IN (SELECT DISTINCT b.bid_id FROM t_invests b WHERE b.id IN (SELECT c.invest_id FROM t_invest_transfers c WHERE c.status = ?))";
		String sql = "SELECT COUNT(DISTINCT a.bid_id) FROM t_bills a WHERE a.overdue_mark <> ? AND (YEAR(a.mark_overdue_time)=? AND MONTH(a.mark_overdue_time)=?) AND a.bid_id IN (SELECT DISTINCT b.bid_id FROM t_invests b WHERE b.id IN (SELECT c.invest_id FROM t_invest_transfers c WHERE c.status = ?))";
		Query query = JPA.em().createNativeQuery(sql)
				.setParameter(1, Constants.BILL_NO_OVERDUE)
				.setParameter(2, year)
				.setParameter(3, month)
				.setParameter(4, Constants.DEBT_SUCCESS);
	 ...
	 public static long querySuccessDebtAmount(int year,int month,ErrorInfo error){
		error.clear();
...
//		String sql = "SELECT COUNT(DISTINCT b.bid_id) FROM t_invests b WHERE b.id IN (SELECT a.invest_id FROM t_invest_transfers a WHERE a.status = ? AND DATE_FORMAT(a.transaction_time,'%y%m') = DATE_FORMAT(CURDATE(),'%y%m'))";
		String sql = "SELECT COUNT(DISTINCT b.bid_id) FROM t_invests b WHERE b.id IN (SELECT a.invest_id FROM t_invest_transfers a WHERE a.status = ? AND (YEAR(a.transaction_time)=? AND MONTH(a.transaction_time)=?))";
		Query query = JPA.em().createNativeQuery(sql)
	 ...
	 public static long querySuccessBidAccount(ErrorInfo error){
		 ...
		 long success_account = 0l;//已放款的投资标总数量
		 Object obj;
		 ...
		 try {
			...
			Logger.error("查询已放款的投资标总数量时：" + e.getMessage());
			
			error.code = -1;
			error.msg = "查询已放款的投资标总数量时异常！";
	 ...
	 * 数据统计--债权转让情况统计分析表
	 */
	public static void debtSituationStatistics(int year,int month){
		 ...
		 long increase_debt_account  = StatisticDebt.queryIncreaseDebtAccount(year,month,error) ;//本月新增转让标数量
		 Double increase_debt_amount_sum = StatisticDebt.queryIncreaseDebtAmountSum(year,month,error) ;//本月新增转让总额
		 long has_overdue_debt = StatisticDebt.queryHasOverdueDebt(year,month,error);//转让债权标含逾期数量
		 double overdue_percent = 0 ;//转让债权逾期占比
		 double average_debt_amount = 0 ;//转让债权均标金额
		 long success_debt_amount = StatisticDebt.querySuccessDebtAmount(year,month,error) ;//债权转让成功标数量
		...
		long temp = StatisticDebt.querySuccessBidAccount(error);//已放款的投资标总数量
