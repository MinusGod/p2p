...
//		news.type_id = this.typeId;
	...
	public static List<t_content_news> queryNewForFront(long type_id, int size, ErrorInfo error) {
		...
		String sql = "select new t_content_news(id, title,content, read_count,image_filename,start_show_time) from t_content_news where "
				+ "type_id = ? and is_use = true and start_show_time <= ? "
				+ " order by start_show_time desc";
	...
	public static PageBean <t_content_news>  queryNewsByTypeId(String typIdStr, String currPageStr,
		...
		String sqlPage = "select new t_content_news(id, title, content, read_count, image_filename, time,start_show_time) from "
				+ "t_content_news news where news.type_id = ? and news.is_use = true and "
				+ "news.start_show_time <= ? " + searchCondition + " order by news.start_show_time desc";
	...
	public static List<News> queryNewsDetail(String idStr, String keyword, ErrorInfo error) {
		...
		if (StringUtils.isNotBlank(keyword)) {
 		...
 		String sqlLast = "select new t_content_news(id,title) from t_content_news " +
 				...
 				"(_order=(select _order from t_content_news where id=?) and start_show_time<(select start_show_time from t_content_news where id=?))) " +
 				searchCondition + "order by _order desc,start_show_time desc";
 		
 		String sqlNext = "select new t_content_news(id,title) from t_content_news " +
 				"where type_id=(select type_id from t_content_news where id=?) " +
 				"and is_use=true and (_order>(select _order from t_content_news where id=?) or " +
 				"(_order=(select _order from t_content_news where id=?) and start_show_time>(select start_show_time from t_content_news where id=?))) " +
 				searchCondition + "order by _order,start_show_time";
	 ...
	 * @return List<Product>
	 */
	public static List<Map<String,String>> queryPrincipleProtection(ErrorInfo error) {
		error.clear();
		
		String sql = "select t.name,n.author,n.title,n.time,n.content,n.image_filename,n.image_filename2 "
				+ "from t_content_news n,t_content_news_types t where n.type_id = t.id "
				+ "and t.parent_id = 4 and n.is_use = true and t.status = 1 and n.start_show_time <= now() order by n.type_id";
		
		List<Object[]> objList = null;
		try {
			objList = (List<Object[]>) JPA.em().createNativeQuery(sql).getResultList();
		} catch (Exception e) {
			e.printStackTrace();
			Logger.error("查询本金保障时："+e.getMessage());
			
			error.code = -1;
			error.msg = "查询本金保障时出现异常";
		...
		for(Object[] obj : objList){
			...
			map.put("author", obj[1] == null ? "":obj[1].toString());
			map.put("title", obj[2] == null ? "":obj[2].toString());
			map.put("time", obj[3] == null ? "":obj[3].toString());
			map.put("content", obj[4] == null ? "":obj[4].toString());
			map.put("image_filename", obj[5] == null ? "":obj[5].toString());
			map.put("image_filename2", obj[6] == null ? "":obj[6].toString());
