...
public class BidPlatformAction extends SupervisorController {
	...
	public static void auditToFundraise(String sign) {
		...
		String managerFee = params.get("managerFee");
		...
		//添加借款管理费率 begin   add by xd.wang
		if(StringUtils.isNotEmpty(managerFee)){
			bid.managerFee = Double.parseDouble(managerFee); 
		}
		//添加借款管理费率  end
		bid.allocationSupervisorId = Supervisor.currSupervisor().id; // 审核人
	...
	public static void auditToNotThrough(String sign) { 
		...
		String suggest = params.get("through");
		...
		if(Constants.IPS_ENABLE && error.code >= 0) {
			...
			PaymentProxy.getInstance().bidAuditFail(error, Constants.PC, bidId, IPSConstants.BID_CANCEL_S, bid.auditSuggest, bid.allocationSupervisorId);	
			flash.error(error.msg);
	...
	public static void advanceLoanToPeviewNotThrough(String sign) { 
		...
		if(Constants.IPS_ENABLE) {
			//流标调用环迅托管接口, 处理业务逻辑在托管实现类回调方法中  error  client  标的id   流标方式
			PaymentProxy.getInstance().bidAuditFail(error, Constants.PC, bidId, IPSConstants.BID_CANCEL_B, bid.auditSuggest, bid.allocationSupervisorId);	
			flash.error(error.msg); 
