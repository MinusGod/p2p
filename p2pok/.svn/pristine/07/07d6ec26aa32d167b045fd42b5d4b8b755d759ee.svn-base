...
public abstract class PaymentBaseService {
	...
	/**
	 * 扩展参数方法-用户汇付天下回调后,平台防重复业务等需要的参数
	 * @param maps
	 * @return
	 */
	public static Map<String,String> extraParams(Map<String,String> maps){
		String cmdId = maps.get("CmdId");
		if(HfConstants.CMD_USERREGISTER.equals(cmdId)){
			maps.put("OrdId", maps.get("MerPriv"));
		}else if(HfConstants.CMD_BATCHREPAYMENT.equals(cmdId)){
			maps.put("UsrCustId", maps.get("OutCustId"));
			maps.put("OrdId", maps.get("BatchId"));
		}else if(HfConstants.CMD_ADDBIDINFO.equals(cmdId)){
			maps.put("OrdId", maps.get("ProId"));
		}else if(HfConstants.CMD_USERBINDCARD.equals(cmdId)){
			maps.put("OrdId", maps.get("MerPriv"));
		}else if(HfConstants.CMD_TRANSFER.equals(cmdId)){
			maps.put("UsrCustId", maps.get("InAcctId"));
		}else if(HfConstants.CMD_BATCHREPAYMENT.equals(cmdId)){
			maps.put("UsrCustId", maps.get("OutCustId"));
			maps.put("OrdId", maps.get("BatchId"));
		}
