...
public class DealDetail implements Serializable{
	 ...
	 * @param relationId 当类别为：1 t_user_audit_items中的id,2  账单的id, 3 借款标的id, 4 t_invest投资id 
	 * -1 逾期账单id
	 * @param summary 摘要说明
	 * @param error
	 */
	...
	public static PageBean<v_supervisor_events> querySupervisorEvents(int currPage, int pageSize,
		...
		sql.append(" order by t_supervisor_events.time desc");
	...
	public static void minusUserFund(long userId, double balance, ErrorInfo error){
		...
		if(row == 0){
			error.code = -2;
			error.msg = "减少用户资金失败";
			Logger.info("减少用户资金失败，可能是用户余额不足");
